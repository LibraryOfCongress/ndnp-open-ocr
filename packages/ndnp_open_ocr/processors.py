...
class PDFProcessor:
    ...
    def transfer_xmp(self):
        ...
        if not self.old_pdf or not os.path.isfile(self.old_pdf):
            logging.info(
                "No source PDF found at %s; skipping XMP metadata transfer "
                "(output PDF is valid but carries no prior NDNP metadata)",
                self.old_pdf,
            )
            return
        try:
    ...
    def postprocess_pdf(self):
        ...
        logging.debug("Ghostscript stdout: %s", result.stdout)
        logging.debug("Ghostscript stderr: %s", result.stderr)
...
class OCRProcessor:
    ...
    def generate_pdf(self):
        ...
        try:
            ...
            # Remove pikePdf .pdf_original backup if one was created (pikepdf only
            # leaves it in some cases; its absence is not an error).
            pdf_original = self.get_postprocessed_pdf_path().replace(".pdf", ".pdf_original")
            if os.path.isfile(pdf_original):
