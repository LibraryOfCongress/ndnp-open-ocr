...
run_fargate:
	docker run -it -e AWS_PROFILE=loc1 -e PYTHONPATH=packages -e OUTPUT_BUCKET_NAME=ndnp-open-ocr-output-bucket-test -v ~/.aws:/root/.aws -v $(PWD):/app -w /app ndnp-open-ocr-container-repo:latest bash
